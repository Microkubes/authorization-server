...
func ShowLoginLoginBadRequest(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.LoginController) (http.ResponseWriter, error) {
	...
	showLoginCtx, _err := app.NewShowLoginLoginContext(goaCtx, req, service)
	if _err != nil {
		panic("invalid test data " + _err.Error()) // bug
	}

	// Perform action
	_err = ctrl.ShowLogin(showLoginCtx)
...
// ShowLoginLoginUnauthorized runs the method ShowLogin of the given controller with the given parameters.
// It returns the response writer so it's possible to inspect the response headers and the media type struct written to the response.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
